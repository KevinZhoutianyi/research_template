...
| `discussions/` | Meeting and advisor discussion notes, one file per discussion, dated. |
...
- **Never replace missing evidence with confident prose.** A gap in the data is a sentence naming the gap and a `(planned)` row added to the `paper.md` Outline naming the section it serves, not a smoother paragraph.
...
- The main body tells the positive story only. Ablations, falsified hypotheses, and "what we do NOT claim" lists go to the appendix — with one exception: a mechanism/analysis section keeps its what-we-do-NOT-claim list in the body (next bullet), because there the over-readings are the claims a reader will most likely carry away.
