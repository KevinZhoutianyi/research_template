...
Structure:
- Introduction has no subsections. Flowing prose, ending with a numbered contributions list. Sentence-level scaffolding for the abstract and the introduction (three abstract shapes, the seven-paragraph intro plan, what a contribution bullet must contain) is in the `/verify-paper` skill's `references/abstract-and-intro-templates.md`; open it before drafting or restructuring either section.
...
Tables: `booktabs` rules only (`\toprule`, `\midrule`, `\bottomrule`), never `\hline`, never a vertical rule, never a double rule. Every column whose value has a better direction carries the direction in its header, in the same cell as the name: `Accuracy (\%) $\uparrow$`, `Latency (ms) $\downarrow$`. A reader who has not read the metrics appendix cannot tell from the page which way a project-specific metric is supposed to move, and a reviewer scanning tables reads headers before prose. Columns where neither direction is better (a model name, a dataset, an operating point) carry no marker; adding one to a neutral column is worse than omitting it from a directional one. Same decimal count down a column, units in the header, text columns left-aligned and numeric columns aligned on the decimal point. Group multi-setting columns with `\multicolumn` plus `\cmidrule`, not a rule between them. One table, one message: a table carrying two unrelated comparisons is two tables. Highlight sparingly, and only the row or cell the caption is about.
