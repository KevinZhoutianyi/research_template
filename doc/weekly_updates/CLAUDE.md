...
The rules for weekly updates live in skills, not here. Two forms exist; **each project picks one
at init** (note the choice below, or delete the unused skill):

- `/weekly-update` -- a Markdown progress report (`update.md`), for sponsors/advisors who read
  asynchronously. Comes with a template and a banned-language check script.
- `/weekly-slides` -- a Beamer deck (`slides.tex`), for projects with a live weekly talk.
...
This project uses: `<fill at init>`
