...
Re-read this before writing any weekly update.

The weekly update is a **PDF progress report** (LaTeX article), not a slide deck and not paper prose. It records what happened this week and what the plan is, for a
reader who tracks the project week to week.
...
- **Every claim is grounded, nothing is invented.** Each number traces to a real
  `results.json` or run log; each mechanism (the causal "why", e.g. "the model over-triggers on short inputs")
...
- LaTeX article at `doc/weekly_updates/YYYY-MM-DD/update.tex`, compiled `update.pdf` committed
  alongside. `\documentclass[11pt]{article}`, one-inch margins, booktabs tables,
  `\good{}`/`\bad{}` green/red for direction-of-change.
...
1. `\section*{What this project is}` -- 3-5 sentences only: what an LLM agent is, what the
   system does, the question this track asks, what model/hardware. No definitions here beyond
   what those sentences need.
2. `\section*{Definitions}` -- **a dedicated section, one bullet per term of art** (game,
...
3. One `\section*{Result N: <plain-language finding with its numbers>}` per subgoal. Section
...
4. `\section*{Plan}` -- every item names its prediction and, where one exists, its falsifier,
   both stated concretely ("if the score still sits at 67--69, then X").
5. `\section*{Summary}` -- **at the END, after Plan, not at the top.** 3-5 bullets, one per
   result section, each cross-referencing its section ("(Result 1)") and carrying the headline
   numbers with baselines. A reader who reads only the Summary knows the week.
6. `\appendix` -- one verbatim example per claim type (a real test case, a rescued game, a
   broken game), quoted from committed run data, never composed.

**Every result table is preceded by two labeled paragraphs:**

- `\textbf{Data.}` -- which dataset, its size, and **exactly what is train and what is test**:
...
- `\textbf{Metric.}` -- what is counted, what a point means, and the floor/ceiling values with
...
- **Titles obey §3 and §4.3 even when they are correct conclusions.** A layperson reading only
  the title must get the finding, so no jargon or metaphor in it: not "component X is load-bearing" but a plain sentence a non-specialist can restate.
- **One variable per comparison.** A row that changes two things at once (stronger model AND new
  ...
  about, and say in the Metric paragraph what is held fixed (state which factor is held constant so the reader sees the one thing that varies). If two things genuinely differ, split into two comparisons.
- **When something does not work, give the failure reason from evidence, not just the number.**
  Not "variant X loses (35.6 vs 48.8)" but the evidenced mechanism (what in the logs shows why it lost). If the reason is not
...
- Compile, render pages to PNG, and read the render (the stranger-read pass from
  `doc/CLAUDE.md`): every table readable, no overfull boxes that clip content, every term used
  in the body present in the Definitions section, every number carrying its baseline.
- Every number matches its `results.json` or `tracking.md` row (three-way consistency).
- Grep the .tex for the banned-metaphor list in §3 before committing.
- Commit both `update.tex` and `update.pdf`.
