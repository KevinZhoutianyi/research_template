...
This file is the **living paper pitch** for the project. Update in place;
do not append dated entries. A reader scanning top-to-bottom should hit:
  Goal → Thesis → Prior work → Outline → Evidence (one §/subgoal) →
  Status → Reframing for slides → Appendices.

The structure below is taken from a real research project and is meant to
generalize. Replace [BRACKETED PLACEHOLDERS] with your content; the
HTML comments explain the *intent* of each block so you know what belongs
there even when your project's specifics differ.
-->
...
<!-- Tables, key numbers, a 2-3 sentence interpretation.
Classify the evidence: Central / Sanity check / Supporting. -->

**Classification:** [Central | Sanity check | Supporting].

| condition | result | interpretation |
|---|---|---|
| [baseline] | [number] | [one-line meaning] |
| [test] | [number] | [one-line meaning] |
...
<!-- Inline status of jobs currently on the cluster. Remove when the job
lands and its result is integrated above. -->

| Exp | Job ID | ETA | Purpose |
|---|---|---|---|
| [Exp X] | [12345] | [tonight] | [closes Q3 in §3] |

## Recently completed (this week)

| Exp | Finding | Integrated into |
|---|---|---|
| [Exp Y] | [one-line headline] | [§N evidence section] |

## Recently failed jobs

<!-- Document failures so they aren't re-attempted blindly. Cite the
diagnostic and the workaround applied (or "not yet fixed"). -->

| Exp | Job ID | Failure mode | Resolution |
|---|---|---|---|
| [Exp Z] | [12300] | [OOM / meta-tensor / path bug] | [fixed in commit X / blocked on Y] |

## Next steps

- [One-line action item with the subgoal it serves.]
- [Action item.]
