...
### 2c.2 Motivation and method figures are generated by the USER via GPT image (workflow)

For agent-paper motivation/method/pipeline figures (the §2c kinds), the render is produced by
the user in ChatGPT's image model, not by matplotlib or LaTeX here. Diffusion renders look
right but garble small text, so the division of labor is strict:

1. **Claude writes a generation prompt, never the image.** The prompt contains: (a) which
   exemplar to attach (`doc/example_papers/example_fig_*.png`) and one line naming the design
   to copy from it; (b) the full layout spec (panels, cards, arrows, colors by role); (c)
   EVERY literal string, each in quotes, marked "render exactly as written" -- lesson/steer
   quotes, task text, step labels, scores, takeaway lines. Strings come from run data,
   verified before they enter the prompt.
2. **The user feeds the prompt (plus the exemplar image) to GPT image** and returns the render.
3. **Claude verifies the render against the string list** before it enters the report: every
...
Data plots (bars, trends, heatmaps: anything with axes and measured numbers) stay in
matplotlib under the house style; typeset tables stay in LaTeX. Only the illustrative
diagram kinds go through this workflow.
